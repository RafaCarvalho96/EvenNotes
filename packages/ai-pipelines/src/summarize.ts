...
const logger = createLogger('SummarizePipeline')
...
export class SummarizePipeline implements PipelineRunner {
  async *run(input: PipelineInput): AsyncIterable<PipelineEvent> {
    ...
    const trace = startTrace('summarize', input.runId)

    try {
      // Stage 1 – Input resolution
      if (!input.content || input.content.trim().length === 0) {
        const error = 'Content is empty. Please provide content to summarize.'
        logger.error('pipeline failed: empty content', { runId: input.runId, pipeline: 'summarize' })
      ...
      logger.info('pipeline completed', { runId: input.runId, pipeline: 'summarize', durationMs, outputLength: output.length })
      endTrace(trace, output)
      yield { type: 'completed', output }
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err)
      const durationMs = Date.now() - startedAt
      logger.error('pipeline failed', { runId: input.runId, pipeline: 'summarize', durationMs, error: message })
